...
plot_condemcom = ggplot(condemcom, aes(x = country, y = total_count)) +
  geom_col(fill = "skyblue", alpha = 0.7, width = 0.5) +  # Column graph for total_count
  labs(title = "Total Count by Country (condemcom)",
       x = "Country",
       y = "Total Count") +
...
plot_conmilcom <- ggplot(conmilcom, aes(x = country, y = percentage)) +
  ...
  labs(title = "Total Count by Country (conmilcom)",
       x = "Country",
       y = "Percentage") +
  scale_y_continuous(expand = c(0, 0))+
  Theme 
...
plot_poldemcom = ggplot(poldemcom, aes(x = original_dataset, y = total_support)) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "firebrick1", linetype = "solid")+
...
plot_polmilcom = ggplot(polmilcom, aes(x = original_dataset, y = percentage)) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "skyblue", linetype = "solid")+
